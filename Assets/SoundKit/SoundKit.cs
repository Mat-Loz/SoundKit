...
public class SoundKit : MonoBehaviour
{
    ...
    /// <param name="audioClip">Audio clip.</param>
    /// /// <param name="volumeScale">Volume.</param>
    public SKSound playSoundLooped(AudioClip audioClip, float volumeScale = 1.0f)
    {
        // find the first SKSound not being used. if they are all in use, create a new one
        SKSound sound = nextAvailableSound();
        sound.playAudioClip(audioClip, volumeScale * soundEffectVolume, 1f, 0f);
        sound.setLoop(true);
    ...
    public void recycleSound(SKSound sound)
    {
        ...
        if (_playingSounds.Count > index)       // NOTE : Added by Mat
